...
function renderArticle(articleData) {
    ...
    articleComment.classList.add("comments-list");
...
function renderComments(article){
    fetch(`https://simple-json-server-scit.herokuapp.com/comments?postId=${article.id}`).then(handleFetchResponse).then(CommentsList)
}
...
function renderingComments(input){
    let postDiv = document.getElementById(input.postId);
    let commentsListDiv = postDiv.querySelector(".comments-list");
    ...
    commentsListDiv.appendChild(comment);
}